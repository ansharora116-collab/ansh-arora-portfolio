...
export const AboutSection: React.FC = () => {
  ...
  return (
    <section 
      id="about" 
      className="relative w-full min-h-viewport bg-black text-[#E8DFD8] font-sans selection:bg-[#cbb59d] selection:text-black py-24 lg:py-32 px-6 sm:px-12 lg:px-20 overflow-hidden flex items-center"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.p
              ...
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              I'm <span className="text-[#F3DBB3] font-medium">{profile.fullName}</span>, {about.bio}
            </motion.p>

            {/* Concise 4-Item Achievement Metrics Grid */}
            <motion.div
              variants={fadeUpVariants}
              className={`grid grid-cols-2 sm:grid-cols-4 gap-6 ${about.stats.length ? 'pt-6 pb-2 border-t border-[#8C6D4F]/25' : ''}`}
            >
              {about.stats.map((stat) => (
                <div key={stat.label} className="flex flex-col">
                  <span
                    className={`text-3xl sm:text-4xl font-light tracking-tight ${stat.gold ? 'text-[#D4AF37]' : 'text-[#F4EBE2]'}`}
                    style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                  >
                    {stat.value}
                  </span>
          ...
          <div className="lg:col-span-5 flex items-center justify-center relative perspective-[1400px]">
            ...
            >
              ...
              <div className="relative overflow-hidden w-full max-w-[390px] aspect-[4/5] bg-black rounded-sm">
                {/* Main Portrait — falls back to a monogram plate until a photo is added */}
                {about.portrait ? (
                  <img
                    src={about.portrait}
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center bg-[radial-gradient(ellipse_at_50%_30%,#1E1913,#0A0806_70%)]">
                    <span
                      className="text-[9rem] leading-none text-[#D4AF37]/30 select-none"
                      style={{ fontFamily: "'Herr Von Muellerhoff', 'Allura', cursive" }}
                    >
                      {profile.monogram}
                    </span>
                    <span
                      className="mt-4 text-[9px] tracking-[0.3em] uppercase text-[#8C6D4F] text-center px-6"
                      style={{ fontFamily: "'Montserrat', sans-serif" }}
                    >
                      Add your portrait to src/assets
                    </span>
                  </div>
                )}
